...
function customerEmailHtml(order: any, items: any[], trackingUrl: string) {
  ...
  <div style="max-width:560px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08)">
    ...
    <div style="background:#f9fafb;border-top:1px solid #e5e7eb;padding:16px 32px;text-align:center">
      <p style="margin:0;font-size:12px;color:#9ca3af">Karen West Natural Spring · Karen, Nairobi · P.O. Box 24563-00502</p>
    </div>
  </div></body></html>`;
}
...
function businessEmailHtml(order: any, items: any[], trackingUrl: string) {
  ...
  <div style="max-width:520px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08)">
    <div style="background:#1e3a5f;padding:20px 28px">
      <h1 style="margin:0;color:#fff;font-size:18px">New Order — ${order.order_number}</h1>
    </div>
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const siteUrl = Deno.env.get("SITE_URL") || "https://karen-west-natural-springs.vercel.app";
    const trackingUrl = `${siteUrl}/track/${order.order_number}`;

    // Email business owner
    const bizEmail = Deno.env.get("BUSINESS_EMAIL") || "uwazi.ke@gmail.com";
    await sendEmail(bizEmail, `New Order ${order.order_number} — ${order.customer_name}`, businessEmailHtml(order, items ?? [], trackingUrl));
    ...
    return new Response(JSON.stringify({ success: true, trackingUrl, customerWhatsappLink }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err) {
